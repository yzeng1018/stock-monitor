...
def get_close_data_us_hk(symbols):
    """获取美股/港股收盘价 + 30天历史（用于条件2、3）"""
    results = []
    for symbol in symbols:
        try:
...
def run_close_check(market):
    ...
    if market == "a":
        ...
        stocks = get_close_data_us_hk(HK_STOCKS)
    else:
        stocks = get_close_data_us_hk(US_STOCKS)
